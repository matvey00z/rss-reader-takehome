...
class DB:
    ...
    def get_or_put_feed(self, cursor, url: str):
        ...
        if result:
            return result[0]
        cursor.execute('INSERT INTO feeds (feed_url) VALUES(%s) RETURNING feed_id', (url,))
    ...
    def unfollow_feed(self, username: str, feed_url: str):
        ...
        with self.conn() as conn:
            with conn.cursor() as cursor:
                ...
                cursor.execute("DELETE FROM UserFeeds WHERE user_id = %s AND feed_id = %s", (user_id, feed_id))
            ...
            with conn.cursor() as cursor:
                ...
                cursor.execute(
                    '''SELECT Feeds.feed_url FROM Feeds
                       ...
                       WHERE UserFeeds.user_id = %s''', (user_id,)
                )
    ...
    def put_updates(self, etag: Optional[str], modified: Optional[str],
            feed_url: str, entries: List[str]):
        ...
        with self.conn() as conn:
            with conn.cursor() as cursor:
                ...
                execute_values(cursor, query,
                    [(feed_id, entry['published'], entry['content']) for entry in entries])
    ...
    def get_feed_items(self, username: str, feed_url: str, unread_only: bool):
        with self.conn() as conn:
            with conn.cursor() as cursor:
                user_id = self.get_user_id(cursor, username)
                feed_id = self.get_feed_id(cursor, feed_url)
                cursor.execute('''SELECT last_read_id FROM UserFeeds
                ...
                results = cursor.execute(f'''SELECT item_id, entry FROM FeedItems
    ...
    def get_all_items(self, username: str, unread_only: bool):
        with self.conn() as conn:
            with conn.cursor() as cursor:
                user_id = self.get_user_id(cursor, username)
                unread_query = 'AND items.item_id > feeds.last_read_item_id' if unread_only else ''
                cursor.execute(f'''
                    SELECT items.entry
    ...
    def mark_as_read(self, username: str, feed_url: str, item_id: int):
        with self.conn() as conn:
            with conn.cursor() as cursor:
                user_id = self.get_user_id(cursor, username)
                feed_id = self.get_feed_id(cursor, feed_url)
                cursor.execute('''UPDATE UserFeeds SET last_read_item_id = %s
                    WHERE user_id = %s AND feed_id = %s''',
                    (user_id, feed_id))
    ...
    def request_feed_update(self, feed_url: str):
        with self.conn() as conn:
            with conn.cursor() as cursor:
                res = cursor.execute('''UPDATE Feeds SET failed = false
                    WHERE feed_url = %s
                    RETURNING failed''', (feed_url,))
                try:
